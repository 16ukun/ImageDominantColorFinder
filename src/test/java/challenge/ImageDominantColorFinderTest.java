...
class ImageDominantColorFinderTest {
	...
	@Test
	void test_without_ignoring_colors() {
        ...
        Set<String> ignoredColors = Set.of("");
	...
	void test_ignoring_Red() {
        ...
        Set<String> ignoredColors = Set.of("250,0,0");
        int numberOfColors = 2;
        ...
        // Assert - Validate the results
        assertEquals(2, result.size());
        assertEquals("250,250,0", result.get(0).getKey()); // Yellow is dominant, because Red is ignored
        assertEquals("0,0,0", result.get(1).getKey()); // Current default order returns Black (even though there are multiple colours)
